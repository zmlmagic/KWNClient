...
static KWNSliderViewController *shareKWNSliderViewController = nil;
...
+ (KWNSliderViewController *)sharedController
{
    @synchronized(self)
    {
        ...
        dispatch_once(&onceToken,
                  ^{
                shareKWNSliderViewController = [[self alloc] init];
            });
        return shareKWNSliderViewController;
    }
}
...
- (void)showShadow:(BOOL)val {
    ...
    if (val) {
        ...
        _root.view.layer.shadowRadius = 6.0f;
...
- (void)setRootController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    if (_menuFlags.showingLeftView) {
        ...
        __block KWNSliderViewController *selfRef = self;
...
{
    [_pan setEnabled:_bool];
    [_tap setEnabled:_bool];
}
